...
class MockAddressRepository {
    ...
    static Address getRandom() {
        String address = ADDRESSES[2];
        String[] aParts = address.split(",");
        String a1 = aParts[0];
        String a2 = aParts[1];
        String[] cityStateAndZipParts = a2.trim().split(" ");
        LinkedList<String> list = Arrays.stream(cityStateAndZipParts).map(String::trim).collect(Collectors.toCollection(LinkedList::new));
